...
void setup() {
  ...
  {
    SerialUSB.print("Device not configured. Please use the configuration sketch.");
    while (1); // stall out forever.
  }

  SerialUSB.println("Hi I'm Alice, Would you like me to request a new token from Bob? (y/n)");
  ...
  {
    SerialUSB.println();
    SerialUSB.println("Okay. Sending request for token now...");
    SerialUSB.println();
  }
  else
  {
    SerialUSB.print("I don't understand.");
    while (1);
  }
  ...
  if (timeout == 0) SerialUSB.print("Timed Out. Bob never responded.");
  else // we didn't time out yet, so let's pull in the token from Bob.
  {
    SerialUSB.print("response time: ");
    SerialUSB.println(50 - timeout);
...
{
  SerialUSB.println("**Copy/paste the following public key (alice's) into the top of Example6_Challenge_Bob sketch.**");
  SerialUSB.println("(Bob needs this to verify her signature)");
  SerialUSB.println();
  SerialUSB.println("uint8_t AlicesPublicKey[64] = {");
  for (int i = 0; i < sizeof(atecc.publicKey64Bytes) ; i++)
  {
    SerialUSB.print("0x");
    if ((atecc.publicKey64Bytes[i] >> 4) == 0) SerialUSB.print("0"); // print preceeding high nibble if it's zero
    SerialUSB.print(atecc.publicKey64Bytes[i], HEX);
...
void printInfo()
{
  ...
  for (int i = 0 ; i < 9 ; i++)
  {
    if ((atecc.serialNumber[i] >> 4) == 0) SerialUSB.print("0"); // print preceeding high nibble if it's zero
    SerialUSB.print(atecc.serialNumber[i], HEX);
  }
  SerialUSB.println();

  SerialUSB.print("Rev Number: \t");
  for (int i = 0 ; i < 4 ; i++)
  {
    if ((atecc.revisionNumber[i] >> 4) == 0) SerialUSB.print("0"); // print preceeding high nibble if it's zero
    SerialUSB.print(atecc.revisionNumber[i], HEX);
  }
  SerialUSB.println();

  SerialUSB.print("Config Zone: \t");
  if (atecc.configLockStatus) SerialUSB.println("Locked");
  else SerialUSB.println("NOT Locked");

  SerialUSB.print("Data/OTP Zone: \t");
  if (atecc.dataOTPLockStatus) SerialUSB.println("Locked");
  else SerialUSB.println("NOT Locked");

  SerialUSB.print("Data Slot 0: \t");
  if (atecc.slot0LockStatus) SerialUSB.println("Locked");
  else SerialUSB.println("NOT Locked");
  ...
  if (atecc.configLockStatus && atecc.dataOTPLockStatus && atecc.slot0LockStatus)
  {
    ...
    {
      SerialUSB.println("Failure to generate Public Key");
      SerialUSB.println();
    }
